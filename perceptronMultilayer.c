...
float **inicializaPesos(float **pesos, int numero_linhas, int numero_colunas)
{
	int i, j;
	pesos = malloc(numero_linhas * sizeof(float *));
	for (i = 0; i < numero_linhas; i++)
	{
		pesos[i] = malloc(numero_colunas * sizeof(float));
	}
	for (i = 0; i < numero_linhas; i++)
	{
...
int main()
{
	...
	inicializaPesos(pesos1, N_LINHAS, N_COLUNAS);
	inicializaPesos(pesos2, N_LINHAS, N_COLUNAS);
